...
class SiteController extends Controller
{
    ...
    {
        $display = 5;
        $sql = "SELECT id,title,DATE_FORMAT(time,'%m-%d') as time FROM news WHERE top_level =0  ORDER BY time DESC LIMIT 0,5";
        $topNews = News::findBySql($sql)->all();
        return $this->render('index', ['topNews' => $topNews]);
    }
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->login()) {
            //return $this->goBack();
            return Yii::$app->getResponse()->redirect(Yii::$app->getUser()->getReturnUrl('/admin'));
        } else {
